...
public class NameAndValueDTO implements Serializable {
	...
	public void createDetails(List<Integer> branchNums, List<BranchDTO> branchs){
		for (int i = 0; i < branchs.size(); i++) {
			BranchDTO branch = branchs.get(i);
			if (!branchNums.contains(branch.getBranchNum())) {
				continue;
			}
			NameAndValueDetailDTO detailDTO = new NameAndValueDetailDTO();
			detailDTO.setBranchName(branch.getBranchName());
			detailDTO.setBranchNum(branch.getBranchNum());
			detailDTO.setBranchNum(branch.getBranchNum());
