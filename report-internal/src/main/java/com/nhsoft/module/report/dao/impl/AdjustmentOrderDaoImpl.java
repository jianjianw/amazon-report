...
import com.nhsoft.module.report.model.AdjustmentOrder;
...
public class AdjustmentOrderDaoImpl extends DaoImpl implements AdjustmentOrderDao {
	...
	public List<Object[]> findCheckMoneyByBranch(String systemBookCode, List<Integer> branchNums, Date dateFrom, Date dateTo) {
		...
		if (dateFrom != null) {
			sb.append("and ckorder.check_order_audit_time >= '" +  DateUtil.getLongDateTimeStr(DateUtil.getMinOfDate(dateFrom)) + "' ");
		}
		if (dateTo != null) {
			sb.append("and ckorder.check_order_audit_time <= '" +  DateUtil.getLongDateTimeStr(DateUtil.getMaxOfDate(dateFrom)) + "' ");
		}
	...
	public List<Object[]> findItemLoss(String systemBookCode, Date dateFrom, Date dateTo) {
		...
		if (dateTo != null) {
			sb.append("and adjustment_order_audit_time <= '" +  DateUtil.getLongDateTimeStr(DateUtil.getMaxOfDate(dateTo)) + "' ");
		}
		sb.append("and adjustment_order_state_code = '3' ");
		sb.append("and detail.item_num is not null ");
		sb.append("group by bs.branch_num, convert(varchar(20), a.adjustment_order_audit_time, 112), a.adjustment_order_cause, detail.item_num ");
		SQLQuery sqlQuery = currentSession().createSQLQuery(sb.toString());
		sqlQuery.setString("systemBookCode", systemBookCode);
		return sqlQuery.list();
	}
